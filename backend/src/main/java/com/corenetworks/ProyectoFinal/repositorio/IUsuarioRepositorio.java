...
public interface IUsuarioRepositorio extends IGeneralRepositorio<Usuario,Integer> {
    @Query(value = "SELECT u.seguidores FROM Usuario u JOIN u.seguidores s WHERE s.nombreUsuario = :nombreUsuario")
    List<Usuario> BuscarSeguidor(@Param("nombreUsuario") String nombreUsuario);
